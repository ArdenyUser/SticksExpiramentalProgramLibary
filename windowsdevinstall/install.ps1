Install-Module PS2EXE
mkdir C:\DEVwindows
cd C:\DEVwindows
wget https://www.python.org/ftp/python/3.10.0/python-3.10.0-amd64.exe
C:\DEVwindows\python-3.10.0-amd64.exe
rm C:\DEVwindows\python-3.10.0-amd64.exe
wget https://github.com/ArdenyUser/SticksExpiramentalProgramLibary/raw/main/JavaSetup8u311.exe
C:\DEVwindows\JavaSetup8u311.exe
cd C:
cd C:\DEVwindows
wget https://github.com/ArdenyUser/SticksExpiramentalProgramLibary/raw/main/windevSDKscr/SDKcompile.ps1
wget https://github.com/ArdenyUser/SticksExpiramentalProgramLibary/raw/main/windevSDKscr/SDKcompiler.py
wget https://github.com/ArdenyUser/SticksExpiramentalProgramLibary/raw/main/windevSDKscr/ExecuterBuilder.py
